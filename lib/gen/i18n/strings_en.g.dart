...
class TranslationsProjectsItemsEn {
	...
	late final TranslationsProjectsItemsMiaEn mia = TranslationsProjectsItemsMiaEn._(_root);
	late final TranslationsProjectsItemsParrotEn parrot = TranslationsProjectsItemsParrotEn._(_root);
	late final TranslationsProjectsItemsToolzyEn toolzy = TranslationsProjectsItemsToolzyEn._(_root);
	late final TranslationsProjectsItemsInvestancoEn investanco = TranslationsProjectsItemsInvestancoEn._(_root);
}
...
// Path: projects.items.mia
class TranslationsProjectsItemsMiaEn {
	TranslationsProjectsItemsMiaEn._(this._root);
	...
	/// en: 'Open-source, local, privacy-first voice dictation for Windows. Press a global hotkey, speak, and MIA types polished text at your cursor in whatever app is focused. Everything runs on your machine (no cloud, no account, no server) and your voice never leaves the device. A fully offline alternative to Wispr Flow.'
	String get description => 'Open-source, local, privacy-first voice dictation for Windows. Press a global hotkey, speak, and MIA types polished text at your cursor in whatever app is focused. Everything runs on your machine (no cloud, no account, no server) and your voice never leaves the device. A fully offline alternative to Wispr Flow.';

	/// en: 'Built as a Tauri 2 desktop app with a Rust core (the engine) and a thin Svelte 5 webview used only for settings, onboarding, and the floating mic HUD. The live loop runs entirely on-device: global push-to-talk capture (cpal, 16 kHz mono), server-side Silero-VAD-gated warm whisper.cpp transcription, deterministic text cleanup (filler removal, spoken punctuation, sentence casing), and system-wide injection at the cursor via Windows SendInput with a clipboard fallback. pt-BR and English are first-class. Anti-hallucination is always on (VAD plus greedy decoding plus stateless per-utterance requests). Ships an optional NVIDIA CUDA engine (roughly 7 to 10x faster), a custom dictionary, voice-triggered snippets, per-app writing styles, first-run onboarding, and signed in-app auto-update (minisign-verified). GitHub Actions auto-bumps, tags, and publishes a signed Windows installer plus latest.json on every push to main.'
	String get details => 'Built as a Tauri 2 desktop app with a Rust core (the engine) and a thin Svelte 5 webview used only for settings, onboarding, and the floating mic HUD. The live loop runs entirely on-device: global push-to-talk capture (cpal, 16 kHz mono), server-side Silero-VAD-gated warm whisper.cpp transcription, deterministic text cleanup (filler removal, spoken punctuation, sentence casing), and system-wide injection at the cursor via Windows SendInput with a clipboard fallback. pt-BR and English are first-class. Anti-hallucination is always on (VAD plus greedy decoding plus stateless per-utterance requests). Ships an optional NVIDIA CUDA engine (roughly 7 to 10x faster), a custom dictionary, voice-triggered snippets, per-app writing styles, first-run onboarding, and signed in-app auto-update (minisign-verified). GitHub Actions auto-bumps, tags, and publishes a signed Windows installer plus latest.json on every push to main.';
}

// Path: projects.items.parrot
class TranslationsProjectsItemsParrotEn {
	TranslationsProjectsItemsParrotEn._(this._root);
	...
	/// en: 'Local AI voice cloner for Windows. Parrot clones a voice from a few seconds of reference audio and speaks any text you type, entirely on your own machine. No accounts, no API keys, no cloud, and your reference audio never leaves your device. Free and open source (Apache-2.0).'
	String get description => 'Local AI voice cloner for Windows. Parrot clones a voice from a few seconds of reference audio and speaks any text you type, entirely on your own machine. No accounts, no API keys, no cloud, and your reference audio never leaves your device. Free and open source (Apache-2.0).';

	/// en: 'Architected as three cooperating processes in one window: a Svelte UI, a Rust/Tauri shell that owns the window and supervises the engine, and a Python FastAPI sidecar running the PyTorch voice model over localhost. Clone-and-speak is built end to end: record or upload a reference sample, save it as a reusable voice profile, then type text and export natural speech as a .wav. Runs on CUDA or CPU (auto-detected) with a VRAM-budgeted GPU worker pool and flush-and-retry recovery. The first launch bootstraps a Python venv and downloads the voice model once, then works offline forever. Tested across all three layers (pytest for the sidecar, vitest plus svelte-check for the frontend, cargo test plus clippy for the shell) with a whole-app IPC smoke test. Signed-updater keypair and CI release pipeline wired.'
	String get details => 'Architected as three cooperating processes in one window: a Svelte UI, a Rust/Tauri shell that owns the window and supervises the engine, and a Python FastAPI sidecar running the PyTorch voice model over localhost. Clone-and-speak is built end to end: record or upload a reference sample, save it as a reusable voice profile, then type text and export natural speech as a .wav. Runs on CUDA or CPU (auto-detected) with a VRAM-budgeted GPU worker pool and flush-and-retry recovery. The first launch bootstraps a Python venv and downloads the voice model once, then works offline forever. Tested across all three layers (pytest for the sidecar, vitest plus svelte-check for the frontend, cargo test plus clippy for the shell) with a whole-app IPC smoke test. Signed-updater keypair and CI release pipeline wired.';
}

// Path: projects.items.toolzy
class TranslationsProjectsItemsToolzyEn {
	TranslationsProjectsItemsToolzyEn._(this._root);
	...
	/// en: 'Toolzy'
	String get name => 'Toolzy';

	/// en: 'Free, open-source native desktop toolbox for everyday file tasks: convert and resize images, work with PDFs (merge, compress, protect, images to and from PDF), convert and edit audio and video, transcribe speech, and download media from a link. Everything runs natively on your machine, so your files never leave your device.'
	String get description => 'Free, open-source native desktop toolbox for everyday file tasks: convert and resize images, work with PDFs (merge, compress, protect, images to and from PDF), convert and edit audio and video, transcribe speech, and download media from a link. Everything runs natively on your machine, so your files never leave your device.';

	/// en: 'Single Tauri 2 desktop app where Rust is the engine (conversion runs natively, not in a browser sandbox) and a thin React plus TypeScript webview calls Rust via invoke(). Image convert and resize use the Rust image and webp crates; PDF read and write use pdfium-render and printpdf with a qpdf sidecar for merge and AES-256 password protection; audio and video editing run through an ffmpeg sidecar; local speech transcription uses whisper.cpp with optional NVIDIA CUDA; the media downloader wraps yt-dlp. Pure helpers (resize math, filename and argument building) are isolated in their own modules with cargo tests, and the React UI holds no conversion logic. Licensing kept clean by design: no AGPL libraries bundled, with each native binary running as a separate sidecar process. CI runs Biome plus cargo test plus app and site builds, gated into a signed, self-updating Windows installer.'
	String get details => 'Single Tauri 2 desktop app where Rust is the engine (conversion runs natively, not in a browser sandbox) and a thin React plus TypeScript webview calls Rust via invoke(). Image convert and resize use the Rust image and webp crates; PDF read and write use pdfium-render and printpdf with a qpdf sidecar for merge and AES-256 password protection; audio and video editing run through an ffmpeg sidecar; local speech transcription uses whisper.cpp with optional NVIDIA CUDA; the media downloader wraps yt-dlp. Pure helpers (resize math, filename and argument building) are isolated in their own modules with cargo tests, and the React UI holds no conversion logic. Licensing kept clean by design: no AGPL libraries bundled, with each native binary running as a separate sidecar process. CI runs Biome plus cargo test plus app and site builds, gated into a signed, self-updating Windows installer.';
}

// Path: projects.items.investanco
class TranslationsProjectsItemsInvestancoEn {
	TranslationsProjectsItemsInvestancoEn._(this._root);
...
extension on Translations {
	dynamic _flatMapFunction(String path) {
		return switch (path) {
			...
			'projects.items.mia.name' => 'MIA',
			'projects.items.mia.description' => 'Open-source, local, privacy-first voice dictation for Windows. Press a global hotkey, speak, and MIA types polished text at your cursor in whatever app is focused. Everything runs on your machine (no cloud, no account, no server) and your voice never leaves the device. A fully offline alternative to Wispr Flow.',
			'projects.items.mia.details' => 'Built as a Tauri 2 desktop app with a Rust core (the engine) and a thin Svelte 5 webview used only for settings, onboarding, and the floating mic HUD. The live loop runs entirely on-device: global push-to-talk capture (cpal, 16 kHz mono), server-side Silero-VAD-gated warm whisper.cpp transcription, deterministic text cleanup (filler removal, spoken punctuation, sentence casing), and system-wide injection at the cursor via Windows SendInput with a clipboard fallback. pt-BR and English are first-class. Anti-hallucination is always on (VAD plus greedy decoding plus stateless per-utterance requests). Ships an optional NVIDIA CUDA engine (roughly 7 to 10x faster), a custom dictionary, voice-triggered snippets, per-app writing styles, first-run onboarding, and signed in-app auto-update (minisign-verified). GitHub Actions auto-bumps, tags, and publishes a signed Windows installer plus latest.json on every push to main.',
			'projects.items.parrot.name' => 'Parrot',
			'projects.items.parrot.description' => 'Local AI voice cloner for Windows. Parrot clones a voice from a few seconds of reference audio and speaks any text you type, entirely on your own machine. No accounts, no API keys, no cloud, and your reference audio never leaves your device. Free and open source (Apache-2.0).',
			'projects.items.parrot.details' => 'Architected as three cooperating processes in one window: a Svelte UI, a Rust/Tauri shell that owns the window and supervises the engine, and a Python FastAPI sidecar running the PyTorch voice model over localhost. Clone-and-speak is built end to end: record or upload a reference sample, save it as a reusable voice profile, then type text and export natural speech as a .wav. Runs on CUDA or CPU (auto-detected) with a VRAM-budgeted GPU worker pool and flush-and-retry recovery. The first launch bootstraps a Python venv and downloads the voice model once, then works offline forever. Tested across all three layers (pytest for the sidecar, vitest plus svelte-check for the frontend, cargo test plus clippy for the shell) with a whole-app IPC smoke test. Signed-updater keypair and CI release pipeline wired.',
			'projects.items.toolzy.name' => 'Toolzy',
			'projects.items.toolzy.description' => 'Free, open-source native desktop toolbox for everyday file tasks: convert and resize images, work with PDFs (merge, compress, protect, images to and from PDF), convert and edit audio and video, transcribe speech, and download media from a link. Everything runs natively on your machine, so your files never leave your device.',
			'projects.items.toolzy.details' => 'Single Tauri 2 desktop app where Rust is the engine (conversion runs natively, not in a browser sandbox) and a thin React plus TypeScript webview calls Rust via invoke(). Image convert and resize use the Rust image and webp crates; PDF read and write use pdfium-render and printpdf with a qpdf sidecar for merge and AES-256 password protection; audio and video editing run through an ffmpeg sidecar; local speech transcription uses whisper.cpp with optional NVIDIA CUDA; the media downloader wraps yt-dlp. Pure helpers (resize math, filename and argument building) are isolated in their own modules with cargo tests, and the React UI holds no conversion logic. Licensing kept clean by design: no AGPL libraries bundled, with each native binary running as a separate sidecar process. CI runs Biome plus cargo test plus app and site builds, gated into a signed, self-updating Windows installer.',
			'projects.items.investanco.name' => 'Investanco',
			'projects.items.investanco.description' => 'Personal investment portfolio tracker for Android and Web. You register what you own (ticker, quantity, average price) across institutions, and Investanco automatically tracks current value, profit/loss, and performance using public market-data APIs. No broker login and no scraping: the only manual data is your holdings, which change rarely.',
			'projects.items.investanco.details' => 'Built on Clean Architecture with flutter_bloc, get_it, and go_router. Firestore is the source of truth and a local Drift (SQLite) cache is rebuilt at sign-in so reads render immediately, then refresh in the background (it runs on web via sqlite3 WASM). Automatic valuation computes per-holding market value, profit/loss, and return, with fixed-income accrual (CDI, Selic, prefixed, IPCA+) modeled as dated cash flows. Aggregates live and historical prices, FX, and rate series from six public sources (brapi.dev, CoinGecko, Finnhub, Tesouro Direto, BCB SGS, AwesomeAPI). Features user-defined allocation classes with target percentages and a rebalancing plan, bulk CSV import for assets and transactions with a review screen, daily snapshots for history, and Firebase Auth (Google) with a single-owner lock. Charts via fl_chart, functional error handling via dartz (Either), i18n via slang, and API tokens injected at build time through dart-define.',
			'resume.title' => 'Resume',
